...
import { LayoutDashboard, List, Zap, Sparkles, FileText, FolderKanban, ArrowLeft } from "lucide-react";
import { DEMO_TEACHER, DEMO_TFGS } from "./data";

const tfgNav = [
  { name: "Resumen", href: "/demo-teacher/tfgs/1", icon: <LayoutDashboard size={16} />, exact: true },
  { name: "Sprints",    href: "/demo-teacher/tfgs/1/sprints", icon: <Zap size={16} /> },
  { name: "Evaluaciones", href: "/demo-teacher/tfgs/1/grades", icon: <Sparkles size={16} /> },
  { name: "Informes IA",  href: "/demo-teacher/tfgs/1/reports", icon: <FileText size={16} /> },
];
    ...
    <div className="flex h-screen bg-[var(--background)] relative overflow-hidden">
      ...
      <aside className="w-56 shrink-0 my-3 ml-3 h-[calc(100vh-24px)] flex flex-col bg-[var(--background)]/70 backdrop-blur-2xl border border-[var(--border)]/40 rounded-2xl relative z-20 overflow-hidden">
        ...
        <nav className="flex-1 p-3 space-y-4 overflow-y-auto">
          <div>
            ...
            <div className="space-y-1">
              <Link href="/demo-teacher/dashboard"
                className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${pathname === "/demo-teacher/dashboard" ? "bg-[var(--primary)] text-[var(--primary-foreground)]" : "text-[var(--foreground)] hover:bg-[var(--accent)]"}`}>
                <LayoutDashboard size={16} /> Dashboard
              </Link>
              <Link href="/demo-teacher/tfgs"
                className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${pathname === "/demo-teacher/tfgs" ? "bg-[var(--primary)] text-[var(--primary-foreground)]" : "text-[var(--foreground)] hover:bg-[var(--accent)]"}`}>
          ...
          <div>
            <p className="text-xs font-semibold text-[var(--muted-foreground)] uppercase tracking-wider px-3 mb-2 truncate">{DEMO_TFGS[0].title}</p>
            <div className="space-y-1">
              {tfgNav.map((item) => {
        ...
        <div className="p-3 border-t border-[var(--border)] space-y-3">
          <div className="flex items-center gap-2 px-2 py-1">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-medium text-[var(--foreground)] truncate">{DEMO_TEACHER.name}</p>
              <p className="text-xs text-[var(--muted-foreground)]">{DEMO_TEACHER.role}</p>
            </div>
          </div>
          <Link href="/demo" className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs text-[var(--muted-foreground)] hover:bg-[var(--accent)] transition-colors">
            <ArrowLeft size={13} /> Cambiar demo
          </Link>
        </div>
      </aside>
      ...
      <main className="flex-1 overflow-y-auto p-6 relative z-10">
        <div className="mb-4 flex items-center gap-2 px-4 py-2 rounded-lg bg-[var(--primary)]/10 border border-[var(--primary)]/30 text-xs text-[var(--primary)] font-medium">
          <Sparkles size={13} /> Vista demo profesor — datos de ejemplo · <Link href="/" className="underline underline-offset-2">Regístrate para usar la plataforma real</Link>
        </div>
        {children}
      </main>
    </div>
